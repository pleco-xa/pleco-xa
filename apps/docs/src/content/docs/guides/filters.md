...
- `filters.chroma({ sr, n_fft })` — the chroma filterbank: Gaussian pitch-class
  bumps with octave weighting and tuning, matching `librosa.filters.chroma`.
...
- `filters.get_window(type, n)` — a window function (`hann`, `hamming`,
  `blackman`). **Periodic** by convention (`fftbins=True`), matching scipy and
  librosa — the correct choice for spectral analysis.
...
- Windows are **periodic** (`2π·i / n`), not symmetric (`/(n-1)`). This matches
  librosa/scipy `fftbins=True`; the symmetric form skews every spectrogram bin.
- The mel filterbank defaults to the **Slaney** scale with area normalization —
  bit-comparable to librosa. Pass `htk: true` for the HTK scale.
