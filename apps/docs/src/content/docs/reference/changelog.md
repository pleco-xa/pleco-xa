...
fixtures generated by a pinned build of librosa, so "parity" stopped being a
marketing word and became a build-breaking test. See the
[Parity ledger](/reference/parity/) for the full verified table.

### Fixture-verified parity across domains
...
> while the surface settles. The parity fixtures are already enforced in CI.
