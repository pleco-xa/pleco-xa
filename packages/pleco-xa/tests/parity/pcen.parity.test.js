...
// librosa.pcen applied to a mel power-spectrogram, defaults throughout:
...
// sqrt-steady-state smoother coefficient, lfilter_zi warmup (delay state =
// 1-b, i.e. M[-1]=1), log-space AGC (eps+M)^-gain, and the expm1/log1p
// compression branch — is exact: feeding librosa's own mel into this pcen
// reproduces the fixture to 9.0e-8 (float64 round-off). End-to-end from
// pleco's mel the max deviation is 1.8e-7 here, so pcen is not the bottleneck
// and the mel-inherited residual is tiny on this signal. rtol 1e-3 / atol
// 1e-4 gates the combined pipeline with ~1000x headroom.
describe('parity: pcen vs librosa.pcen (on melspectrogram)', () => {
  const fx = loadFixture('pcen')

  for (const c of fx.cases) {
    const { sr, n_fft, hop_length, n_mels } = c.input
    it(`pcen(melspectrogram(n_mels=${n_mels})) matches librosa`, () => {
