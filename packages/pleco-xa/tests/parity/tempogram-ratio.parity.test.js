...
/**
 * Parity gate against librosa 0.11.0
 * (tools/parity/fixtures/tempogram_ratio.json):
 *
 *   tgr = librosa.feature.tempogram_ratio(y=y, sr=22050, hop_length=512)
 *
 * with the default Prockup'15 13-factor table, expected shape [13, 173].
 *
 * The feeding tempogram is itself parity-gated (tempo_beats.json), so any
 ...
 * xa-harmonic.f0_harmonics does NOT reproduce librosa on this grid — the
 * tempo axis is descending with a +Inf head, which that helper brackets
 * ascending-in-place and fills as out-of-bounds, returning all-zeros. So
 * tempogram_ratio interpolates with its own librosa-faithful static-grid
...
describe('parity: tempogram_ratio vs librosa.feature.tempogram_ratio', () => {
  ...
  it('matches librosa shape [13, 173]', () => {
  ...
  it('matches librosa values (rtol 2e-3, atol 2e-3)', () => {
    const out = tempogram_ratio({ y, sr, hop_length })
