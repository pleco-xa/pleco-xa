...
describe('parity: effects.trim / effects.split vs librosa', () => {
  const fx = loadFixture('effects')

  it('trim returns librosa-exact indices (top_db=30)', () => {
  ...
  it('split returns librosa-exact intervals (top_db=30)', () => {
...
describe('parity: effects.preemphasis vs librosa', () => {
  const fx = loadFixture('effects')

  it('matches librosa exactly (coef=0.97, default zi=2*y[0]-y[1])', () => {
  ...
  it('deemphasis(preemphasis(x)) round-trips (librosa guarantee)', () => {
  ...
  it('block streaming chains via zf like librosa', () => {
...
describe('parity: phase_vocoder vs librosa (n_fft=512, hop=128)', () => {
  ...
  for (const c of fx.cases) {
    it(`rate=${c.input.rate}: shape exact, magnitudes within 1e-3, complex within 1e-3 of spectral peak`, () => {
      ...
      expect(worstMag.ratio, `magnitude parity worst offender at f=${worstMag.f},t=${worstMag.t}`).toBeLessThan(1)
      expect(worstCpx.ratio, `complex parity worst offender at f=${worstCpx.f},t=${worstCpx.t} (|dz|=${worstCpx.dev})`).toBeLessThan(1)
    })
  }
...
describe('parity: time_stretch output contract', () => {
  const fx = loadFixture('phase_vocoder')
