 ...
 * constant pad — librosa defaults).
 *
 * Why not the production stft? The fixture descriptors were computed by
 * librosa from ITS |stft|. pocketfft's noise floor in near-silent bins is
 * coherent with the exact result (bandwidth moves <2e-5 between librosa's
 * f32 and f64 STFTs), while our production f32 recursive FFT has a larger
 * incoherent floor (~3e-5 abs) that bandwidth/contrast amplify on
 * quasi-pure tones. An f64 DFT here reconstructs librosa's S faithfully so
 * the fixtures gate the DESCRIPTOR math tightly; the production-stft
 * end-to-end path is exercised separately below at a looser tolerance.
 * (Verified: librosa fed our S reproduces our outputs to the last digit.)
 */
...
describe('parity: spectral descriptors vs librosa.feature.spectral_* (S input)', () => {
  ...
  for (const c of fx.cases) {
    ...
    it(`rms(S=${signal}) matches librosa`, () => {
      const out = rms(null, { S })
      expectClose(out, c.rms, { label: 'rms', rtol: 1e-6, atol: 1e-8 })
    })

    it(`zero_crossing_rate(${signal}) matches librosa`, () => {
...
describe('parity: y-input path through the production f32 stft', () => {
  const fx = loadFixture('spectral_features')
