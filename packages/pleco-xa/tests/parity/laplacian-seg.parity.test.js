...
// The full plot_segmentation pipeline chains a CQT that pleco only approximates,
// so we cross-check the SPECTRAL-CLUSTERING HALF against librosa's own primitives
// (recurrence_matrix + timelag_filter + scipy laplacian/eigh + sklearn KMeans) on
// CONTROLLED two-feature input. Boundaries are permutation- and eigenvector-sign-
// invariant, so exact boundary agreement is a real parity check.
...
describe('parity: laplacianSegmentation two-feature form (McFee-Ellis)', () => {
  ...
  it('recovers librosa boundaries exactly from separate recurrence/path features', () => {
