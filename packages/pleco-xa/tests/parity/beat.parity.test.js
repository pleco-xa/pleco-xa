...
/**
 * Parity gates against librosa 0.11.0 (tools/parity/fixtures/tempo_beats.json):
 *   - expected_tempo:      librosa.feature.tempo(y, sr, hop=512)
 *   - expected_beat_tempo: tempo returned by librosa.beat.beat_track
 *   - expected_beats:      beat frames from librosa.beat.beat_track (hop=512)
 *
 * The fixture values ARE the target (117.45 for the 120-BPM click is
 * librosa's lag-quantized answer — we match librosa, not the nominal BPM).
 *
 ...
 */
describe('parity: tempo/beat_track vs librosa.feature.tempo + librosa.beat.beat_track', () => {
  const fx = loadFixture('tempo_beats')

  for (const c of fx.cases) {
    const { signal, sr, true_bpm } = c.input

    it(`tempo(${signal}) matches librosa.feature.tempo within 2% (achieved: exact)`, () => {
      ...
      // Achieved level: bit-exact lag-bin agreement with librosa
      expect(est).toBeCloseTo(c.expected_tempo, 10)
    })

    it(`beat_track(${signal}) matches librosa tempo + beat frames (achieved: exact frames)`, () => {
      const y = new Float32Array(c.input.y)
  ...
  it('beat_track on silence returns 0 BPM and no beats (librosa semantics)', () => {
