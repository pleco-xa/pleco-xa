...
describe('parity: chroma vs librosa (filters.chroma + feature.chroma_stft)', () => {
  const fx = loadFixture('chroma')

  for (const c of fx.cases) {
    if (c.input.signal === '__filterbank__') {
      it('filters.chroma(sr=22050, n_fft=2048) matches librosa filterbank', () => {
      ...
      it(`chroma_stft(${signal}) matches librosa (incl. estimated tuning)`, () => {
        ...
        // librosa's 0.02 histogram bin). Achieved: 1.8e-6 rel / 2.5e-7 abs.
