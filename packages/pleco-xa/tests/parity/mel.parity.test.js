...
describe('parity: mel filterbank vs librosa.filters.mel', () => {
  const fx = loadFixture('mel_filterbank')
  for (const c of fx.cases) {
    const { sr, n_fft, n_mels, htk, norm } = c.input
    it(`mel(sr=${sr}, n_fft=${n_fft}, n_mels=${n_mels}, htk=${htk}, norm=${norm}) matches librosa`, () => {
      ...
      // Deterministic filter math: only f32 quantization separates us from librosa
...
describe('parity: melspectrogram vs librosa.feature.melspectrogram', () => {
  const fx = loadFixture('melspectrogram')
  for (const c of fx.cases) {
    const { signal, sr, n_fft, hop_length, n_mels } = c.input
    it(`melspectrogram(${signal}) matches librosa (n_fft=${n_fft}, hop=${hop_length}, n_mels=${n_mels})`, () => {
