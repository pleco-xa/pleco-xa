...
/** Frames librosa is confident about — allow the two 220↔330 boundary frames
 *  and the first/last silence-onset frame to differ (pyin is probabilistic and
 *  voicing is genuinely ambiguous exactly at a pitch/silence transition). */
...
describe('pyin parity vs librosa 0.11 (probabilistic YIN / HMM-Viterbi)', () => {
  const c = fixture.cases[0]
  const { fmin, fmax, frame_length, hop_length } = fixture.meta.params
  ...
  it('produces librosa-shaped typed-array outputs', () => {
  ...
  it('voiced/unvoiced classification matches librosa off the transition frames', () => {
  ...
  it('unvoiced f0 is NaN, voiced f0 is finite (librosa fill_na=NaN contract)', () => {
  ...
  it('voiced f0 is within ~1 semitone of librosa on the clear tones', () => {
    const exp = c.expected_f0
    const expVoiced = c.expected_voiced
    let worst = { i: -1, semi: 0 }
    for (let i = 0; i < f0.length; i++) {
      // Only score frames librosa voiced AND we voiced AND not a boundary.
