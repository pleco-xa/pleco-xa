...
describe('golden: decompose.hpss vs committed reference fixture (magnitude S, n_fft=512, hop=128)', () => {
  ...
  it('default margin: H and P match the reference (masked components)', () => {
    const { harmonic: H, percussive: P } = hpss(S)
    // achieved worst abs deviation: 1.61e-5 (inherits stft float32 residual)
    expectClose(flatten(H), c.H, { rtol: 1e-3, atol: 1e-4, label: 'hpss-H' })
    expectClose(flatten(P), c.P, { rtol: 1e-3, atol: 1e-4, label: 'hpss-P' })
  })

  it('margin=2.0: H and P match the reference', () => {
...
describe('unit: softmask (reference semantics)', () => {
  it('matches the reference docstring example (power=2)', () => {
  ...
  it('power=Infinity gives a hard mask with ties favoring the reference input', () => {
...
describe('golden: waveform-level effects.hpss (stft → mask → istft)', () => {
  const y = new Float32Array(c.input.y)
  ...
  it('harmonic + percussive ≈ y with matched length', () => {
    ...
    // golden-gated istft round-trip error; edges of the OLA are the loosest)
    let worst = 0
