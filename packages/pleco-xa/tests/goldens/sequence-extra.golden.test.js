...
describe('sequence golden vs committed reference fixtures (transition_* + viterbi_discriminative)', () => {
  for (const c of fixture.cases) {
    ...
    it(`${fn}: matrix within 1e-6 of the reference`, () => {
  ...
  // the prior (the reference behavior) from multiplying by it. This golden
  // pins the Bayes direction: with p_state=[0.9, 0.1] the likelihood
  // P(state|obs)/p_state makes state 1 win both frames despite its lower raw
  // posterior. A multiply-by-prior implementation would decode [0, 0].
  it('viterbi_discriminative divides by p_state (reference Bayes correction)', () => {
