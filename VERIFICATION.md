...
under [`tools/goldens/`](tools/goldens/README.md) (reference ground truth
captured at a known-good point); the suites that replay them live in
[`packages/pleco-xa/tests/goldens/`](packages/pleco-xa/tests/goldens/).
...
Loop detection — the signature capability — has its own lock, on **real WAVs**,
...
([`examples/node/loop-fast.mjs`](examples/node/loop-fast.mjs)) additionally
checks **BPM within ±0.1** of the pinned value — and measures all four files
landing within **Δ ≤ 1 sample** of the pinned loop points, with BPM exact.
Confidence is asserted on the unified 0..1 scale but deliberately **not**
pinned (the legacy pipeline pegged it at 1.0; the fixture meta documents this).
...
| 10-minute 44.1 kHz track | Valid: `beat_track` in ~30 s and `melspectrogram` in ~22 s under the default Node heap (iterative in-place FFT since 2.0.3 — the former long-input crash is eliminated). |
...
- **Fixtures are frozen snapshots.** They pin the output at a known-good
  ...
  **maintainer operation**, done only when an intentional algorithm change
  ...
  ([`tools/goldens/README.md`](tools/goldens/README.md)).
